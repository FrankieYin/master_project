...
class SDFNet(nn.Module):
    def __init__(self, latent_code_size=LATENT_CODE_SIZE):
        ...
        self.layers1 = nn.Sequential(
            ...
            nn.Linear(3 + latent_code_size, SDF_NET_BREADTH),
            nn.ReLU(inplace=True),

            # nn.utils.weight_norm(nn.Linear(SDF_NET_BREADTH, SDF_NET_BREADTH)),
            nn.Linear(SDF_NET_BREADTH, SDF_NET_BREADTH),
            nn.ReLU(inplace=True),
        ...
        self.layers2 = nn.Sequential(
            # nn.utils.weight_norm(nn.Linear(SDF_NET_BREADTH + latent_code_size + 3, SDF_NET_BREADTH)),
            nn.Linear(SDF_NET_BREADTH + 3 + latent_code_size, SDF_NET_BREADTH),
            nn.ReLU(inplace=True),
